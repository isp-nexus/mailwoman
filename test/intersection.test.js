...
const testcase = (test, common) => {
	...
	assert("Main St & Second Ave", [{ street: "Main St" }, { street: "Second Ave" }])

	assert("Main St @ Second Ave", [{ street: "Main St" }, { street: "Second Ave" }])

	assert("Main St and Second Ave", [{ street: "Main St" }, { street: "Second Ave" }])

	assert("12th Avenue and California Street", [{ street: "12th Avenue" }, { street: "California Street" }])

	assert("15th Avenue and Fulton Street", [{ street: "15th Avenue" }, { street: "Fulton Street" }])

	assert("17th Avenue & Anza Street", [{ street: "17th Avenue" }, { street: "Anza Street" }])

	assert("Gleimstraße an der ecke von Schönhauser Allee", [{ street: "Gleimstraße" }, { street: "Schönhauser Allee" }])

	assert("Gleimstraße und Schönhauserallee", [{ street: "Gleimstraße" }, { street: "Schönhauserallee" }])
	...
	// no street suffix
	assert("foo & bar", [{ street: "foo" }, { street: "bar" }])
	assert("foo and bar", [{ street: "foo" }, { street: "bar" }])
	assert("foo at bar", [{ street: "foo" }, { street: "bar" }])
	assert("foo @ bar", [{ street: "foo" }, { street: "bar" }])

	// missing street suffix - alpha
	assert("main st & side ave", [{ street: "main st" }, { street: "side ave" }])
	assert("main st & side", [{ street: "main st" }, { street: "side" }])
	assert("main & side ave", [{ street: "main" }, { street: "side ave" }])
	assert("main & side", [{ street: "main" }, { street: "side" }])

	// missing street suffix - ordinal
	assert("1st st & 2nd ave", [{ street: "1st st" }, { street: "2nd ave" }])
	assert("1st st & 2nd", [{ street: "1st st" }, { street: "2nd" }])
	assert("1st & 2nd ave", [{ street: "1st" }, { street: "2nd ave" }])
	assert("1st & 2nd", [{ street: "1st" }, { street: "2nd" }])

	// missing street suffix - cardinal
	assert("1 st & 2 ave", [{ street: "1 st" }, { street: "2 ave" }])
	assert("1 st & 2", [{ street: "1 st" }, { street: "2" }])
	assert("1 & 2 ave", [{ street: "1" }, { street: "2 ave" }])
	assert("1 & 2", [{ street: "1" }, { street: "2" }])

	assert("SW 6th & Pine", [{ street: "SW 6th" }, { street: "Pine" }])

	assert("9th and Lambert", [{ street: "9th" }, { street: "Lambert" }])

	assert("filbert & 32nd", [{ street: "filbert" }, { street: "32nd" }])

	// Should not detect this as an intersection
	// assert('University of Hawaii at Hilo', [
	//   { street: 'SW 6th' }, { street: 'Pine' }
	// ])
	assert("national air and space museum", [{ venue: "national air and space museum" }])
	...
	assert("Stonecroft Blvd At Westfields Blvd, Chantilly, VA 20152, USA", [
		{ street: "Stonecroft Blvd" },
		{ street: "Westfields Blvd" },
		{ locality: "Chantilly" },
		{ region: "VA" },
		{ postcode: "20152" },
		{ country: "USA" },
	...
	assert("E Riverside Dr At Montopolis Dr, Austin, TX 78741, USA", [
		{ street: "E Riverside Dr" },
		{ street: "Montopolis Dr" },
		{ locality: "Austin" },
		{ region: "TX" },
		{ postcode: "78741" },
		{ country: "USA" },
