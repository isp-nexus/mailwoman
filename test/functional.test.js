...
const testcase = (test, common) => {
	...
	// address simple
	assert("1 main pl", [{ housenumber: "1" }, { street: "main pl" }])

	// address with ordinal
	assert("100 10th ave", [{ housenumber: "100" }, { street: "10th ave" }])

	// address with cardinal
	assert("100 10 ave", [{ housenumber: "100" }, { street: "10 ave" }])

	// address with directional
	assert("1 north main blvd", [{ housenumber: "1" }, { street: "north main blvd" }])
	assert("1 main blvd north", [{ housenumber: "1" }, { street: "main blvd north" }])

	// address with directional & ordinal
	assert("30 west 26th street", [{ housenumber: "30" }, { street: "west 26th street" }])
	...
	assert("Portland Cafe Portland OR", [{ venue: "Portland Cafe" }, { locality: "Portland" }, { region: "OR" }])
	...
	assert("new south wales aus", [{ region: "new south wales" }, { country: "aus" }])
